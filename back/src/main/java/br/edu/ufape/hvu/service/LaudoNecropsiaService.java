...
public class LaudoNecropsiaService implements LaudoNecropsiaServiceInterface {
	...
	public LaudoNecropsia findLaudoNecropsiaById(long id) {
		return repository.findById(id).orElseThrow( () -> new RuntimeException("It doesn't exist LaudoNecropsia with id = " + id));
	}
	...
	public void deleteLaudoNecropsia(long id){
		LaudoNecropsia obj = repository.findById(id).orElseThrow( () -> new RuntimeException("It doesn't exist LaudoNecropsia with id = " + id));
