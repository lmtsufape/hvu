...
const ClinicaMedicaRetornoPDF = ({ ficha, animal, tutor, medicoLogado }) => {
    const { ExameFisico = {}, fisicogeral = {}, diagnostico = {} } = ficha;

    const vacinas = [
        { key: 'antiRabica', label: 'Anti-Rábica' },
        { key: 'giardia', label: 'Giardia' },
        { key: 'leishmaniose', label: 'Leishmaniose' },
        { key: 'tosseDosCanis', label: 'Tosse dos Canis' },
        { key: 'polivalenteCanina', label: 'Polivalente Canina' },
        { key: 'polivalenteFelina', label: 'Polivalente Felina' },
        ...
        <Document>
            <Page size="A4" style={styles.page}>
                ...
                {/* Dados do Paciente */}
                      <View style={styles.section}>
                        ...
                        <View style={styles.subsection}>
                          ...
                          <View style={styles.row}><Text style={styles.label}>Número de prontuário:</Text><Text style={styles.value}>{animal.codigoProntuario || 'N/A'}</Text></View>
                ...
                <View style={styles.subsection}>
                    ...
                    <Text style={styles.textAreaContent}>{ficha.anamneseHistoricoClinico || 'N/A'}</Text>
                </View>

                <Text style={styles.sectionTitle}>Vacinação, Vermifugação e Ectoparasitos</Text>
                <View style={styles.subsection}>
                ...
                <View style={styles.subsection}><Text style={styles.label}>Mucosas:</Text>{Object.keys(ficha.option || {}).filter(key => ficha.option[key]).map(key => <Text key={key} style={{...styles.value, textAlign: 'left', marginTop: 4}}>• {key}: {ficha.mucosas?.[key] || 'Não especificado'}</Text>)}</View>
                <View style={styles.subsection}><Text style={styles.label}>Linfonodos:</Text>{Object.keys(ficha.linfonodos || {}).map(key => <Text key={key} style={{...styles.value, textAlign: 'left', marginTop: 4}}>• {linfonodosLabels[key]}: {(ficha.linfonodos[key] || []).join(', ')}</Text>)}</View>
...
export default ClinicaMedicaRetornoPDF;
